...
class user(bases.Baseuser):
    ...
    first_name: _str
    last_name: _str
    profile_url: Optional[_str] = None
    user_id: _str
    ...
    @field_validator('artists', pre=True, allow_reuse=True)
    @classmethod
...
class review(bases.Basereview):
    ...
    @field_validator('images', pre=True, allow_reuse=True)
    @classmethod
    def _transform_required_list_fields(cls, value: object) -> object:
        # When using raw queries, some databases will return `None` for an array field that has not been set yet.
        #
    ...
    def create_partial(
        ...
        try:
            ...
            if relations:
                raise ValueError('Model: "review" has no relational fields.')
        except KeyError as exc:
            raise ValueError(
...
_review_relational_fields: Set[str] = set()  # pyright: ignore[reportUnusedVariable]
